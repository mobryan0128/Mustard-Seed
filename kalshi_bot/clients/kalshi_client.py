...
class KalshiMarketPosition:
    """Minimal normalized current market position used for live risk reconciliation."""
...
class KalshiClient:
    ...
    def get_positions(
        ...
        *,
        ...
        if limit <= 0 or limit > 1000:
            raise KalshiClientError("position reconciliation limit must be between 1 and 1000.")
        query_params = {
...
def _normalize_position_payload(payload: object) -> KalshiMarketPosition:
    ...
    return KalshiMarketPosition(
        ticker=_require_text(payload.get("ticker"), "ticker"),
        position_fp=_optional_decimal(payload.get("position_fp")) or Decimal("0"),
...
def _optional_int(value: object) -> int | None:
    ...
    if isinstance(value, bool):
        raise KalshiClientError("Kalshi position payload contained an invalid integer field.")
    try:
        return int(value)
    except (TypeError, ValueError) as exc:
        raise KalshiClientError("Kalshi position payload contained an invalid integer field.") from exc
